...
"""
    ...
    # 2. Rewrite the query if it's on-topic
    rewriter_prompt = f"""
    Conversation History:
    {conversation_history}
    ...
    question that incorporates all necessary context.
    """
    rewritten_query = await nova_model.nova_content_generation(
        ...
        system_message=(
            ...
            "standalone question based on the conversation history."
        ),
    )
...
async def main():
    ...
    # Flooding-related (on-topic) example
    query4 = "How can communities prepare for more frequent flooding?"
    rewritten_query4 = await query_rewriter(history, query4, model)
    print(f"Original Query 4: {query4}")
    print(f"Rewritten Query 4: {rewritten_query4}")
